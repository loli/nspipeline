...
## Changelog
# 2010-10-15 changed the ADC creation script and added a conversion of non-float to float images
# 2010-10-02 created
...
# main code
log 2 "Linking images from collection ${c01dir}" "[$BASH_SOURCE:$FUNCNAME:$LINENO]"
for ((i = 0; i < ${#c01indicessrc[@]}; i++)); do
	...
	for s in "${sequences[@]}"; do
		...
		# link files otherwise
		lncond ${c01dir}/${c01indicessrc[$i]}/${s}.${imgfiletype} ${originals}/${c01indicestrg[$i]}/${s}.${imgfiletype}
	done
done
