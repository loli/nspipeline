...
def plot_maxfeatures(x, x_label, y, y_labels):
    ...
    # plot data
    __plot_multiple(ax, x, x_label, y, y_labels, MARKER_W_LINE, [c_face_1, c_face_2, c_face_4], ['k', 'k', 'k'], 10)
...
def plot_sequence_combinations(filename, x, x_label, y, y_labels):
    ...
    rect1 = matplotlib.patches.Rectangle((-0.5, 0),  5, 1, color=green_lighter, alpha=0.5)
    rect2 = matplotlib.patches.Rectangle(( 4.5, 0), 10, 1, color=green_light, alpha=0.5)
    rect3 = matplotlib.patches.Rectangle((14.5, 0), 10, 1, color=green_lighter, alpha=0.5)
    rect4 = matplotlib.patches.Rectangle((24.5, 0), 5, 1, color=green_light, alpha=0.5)
    rect5 = matplotlib.patches.Rectangle((29.5, 0), 1, 1, color=green_lighter, alpha=0.5)
    rect_flair = matplotlib.patches.Rectangle((-0.5, 0.6), len(y[0]), 0.1, color='#000000', alpha=0.1)
    ax.add_patch(rect1)
    ax.add_patch(rect2)
    ax.add_patch(rect3)
    ax.add_patch(rect4)
    ...
    # default value
    ax.axhline(0.631, linewidth=1, color=c_highlight_line, linestyle='--')
    
    # plot data
    __plot_multiple(ax, x, x_label, y, y_labels, ['o'], [c_face_1], ['#000000'], 10)
        
    # other changes to the figure
    ax.set_ylabel(y_labels[0])
    plt.xticks(range(len(combinations)), ['\n'.join(_x) for _x in combinations], fontsize='x-small', family='monospace')
...
def plot_feature_combinations(filename, x, x_label, y, y_labels):
    # prepare x-axes labels
    ...
    # prepare and draw background rectangles
    ## Rectangle( (x,y)-corner, width, height, color=name|hexcode, alpha=float)
    rect1 = matplotlib.patches.Rectangle((-0.5, 0), 4, 1, color=green_lighter, alpha=0.5)
    rect2 = matplotlib.patches.Rectangle(( 3.5, 0), 6, 1, color=green_light, alpha=0.5)
    rect3 = matplotlib.patches.Rectangle(( 9.5, 0), 4, 1, color=green_lighter, alpha=0.5)
    rect4 = matplotlib.patches.Rectangle((13.5, 0), 1, 1, color=green_light, alpha=0.5)
    ax.add_patch(rect1)
    ax.add_patch(rect2)
    ax.add_patch(rect3)
    ax.add_patch(rect4)
    ...
    # default value
    ax.axhline(0.650, linewidth=1, color=c_highlight_line, linestyle='--')
...
def __plot_multiple(ax, x, xl, y, yl, markers, colors, edges, msize):
    for idx in range(len(y)):
        ...
        ax.plot(_x, _y, markers[idx], label=yl[idx], color=colors[idx], markeredgecolor=edges[idx], markersize=msize)
